...
Angled Reach contains episode-0 comparisons for Ketchup, Drill, and Macaroni (`AngledReachMacaroniTask`). Ketchup compares V-JEPA 2-AC*, side-view V-JEPA 2, wrist-view V-JEPA 2, and DUET-DINO. Drill compares wrist-view DINOv3, independent DINOv3, and DUET-DINO. Macaroni compares side-view DINOv3, side-view V-JEPA 2, and DUET-DINO. Wrist-only models display only the wrist camera; independent DINOv3 and DUET-DINO display the global camera with a bottom-left wrist inset. Symbols follow episode-0 logs, and exact sources are recorded in `static/media/results-angled-reach-manifest.json`.
